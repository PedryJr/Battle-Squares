...
public sealed class FlagBehaviour : MonoBehaviour, ISync
{
    ...
    FlagActivityState activityState = FlagActivityState.Idle;
    ...
    {
        ProjectileBehaviour projectileBehaviour = collision.GetComponent<ProjectileBehaviour>();

        if (projectileBehaviour)
        {
            if (projectileBehaviour.IsLocalProjectile)
            {
