...
public sealed class PlayerController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        { InputHandler(() => { upInputDirection = new Vector2(0, 1f); }); };
        inputs.SquareController.Up.canceled += (context) => 
        { InputHandler(() => { upInputDirection = new Vector2(0, 0); }); };

        inputs.SquareController.Down.performed += (context) => 
        { InputHandler(() => { downInputDirection = new Vector2(0, -1f); }); };
        inputs.SquareController.Down.canceled += (context) => 
        { InputHandler(() => { downInputDirection = new Vector2(0, 0); }); };

        inputs.SquareController.Left.performed += (context) => 
        { InputHandler(() => { leftInputDirection = new Vector2(-1, 0); }); };
        inputs.SquareController.Left.canceled += (context) => 
        { InputHandler(() => { leftInputDirection = new Vector2(0, 0); }); };

        inputs.SquareController.Right.performed += (context) => 
        { InputHandler(() => { rightInputDirection = new Vector2(1, 0); }); };
        inputs.SquareController.Right.canceled += (context) => 
        { InputHandler(() => { rightInputDirection = new Vector2(0, 0); }); };

        inputs.SquareController.Jump.performed += (context) => 
        { InputHandler(() => { if (playerBehaviour.hasJump) { inputJump = true; playerBehaviour.hasJump = false; } }); };

        inputs.SquareController.PrimaryConst.performed += (context) =>
        { InputHandler(() => { shootPrimary = true; }); };
        inputs.SquareController.SecondaryConst.performed += (context) =>
        { InputHandler(() => { shootSecondary = true; }); };

        inputs.SquareController.PrimaryConst.canceled += (context) =>
        { InputHandler(() => { shootPrimary = false; }); };
        inputs.SquareController.SecondaryConst.canceled += (context) =>
        { InputHandler(() => { shootSecondary = false; }); };

        inputs.SquareController.Primary.performed += (context) =>
        { InputHandler(() => { if (uiRegs != 0) return; shootPrimary = true; }); };
        inputs.SquareController.Secondary.performed += (context) =>
        { InputHandler(() => { if (uiRegs != 0) return; shootSecondary = true; }); };
